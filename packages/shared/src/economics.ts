...
/** USD micros for `lamports` at `solPriceUsd`, exact in bigint (price rounded to micro-dollars). */
export const usdMicrosFromLamports = (lamports: bigint, solPriceUsd: number): bigint =>
  (lamports * BigInt(Math.round(solPriceUsd * 1e6))) / 1_000_000_000n;

/** Lamports needed to buy `usdMicros` worth of SOL at `solPriceUsd`. */
export const lamportsFromUsdMicros = (usdMicros: bigint, solPriceUsd: number): bigint =>
  (usdMicros * 1_000_000_000n) / BigInt(Math.round(solPriceUsd * 1e6));

export const formatUsdMicros = (micros: bigint): string => `$${(Number(micros) / 1e6).toFixed(2)}`;

/** Share of the launcher cut that goes to $BERTH stakers of the app, when any exist. */
...
export function splitFees(usdMicros: bigint, hasParent: boolean, hasStakers: boolean): FeeSplit {
  ...
  const shipMicros = bps(usdMicros, FEE_SPLIT_BPS.SHIP_TOKEN);
  let launcherMicros = usdMicros - buildMicros - shipMicros;
  ...
  return { usdMicros, buildMicros, creditsMicros, shipMicros, launcherMicros, upstreamMicros, stakersMicros };
}
